...
def load_fst(filepath):
    """Load PLINK .fst file â†’ DataFrame with CHR, SNP, POS, FST."""
...
def load_freq(filepath):
    """Load PLINK .frq file â†’ DataFrame with SNP, MAF, A1, A2."""
...
def calculate_pbs(t_ab, t_ac, t_bc):
    ...
    Positive PBS â†’ branch lengthening on UZB lineage â†’ possible selection
    """
...
def main():
    ...
    print(f"\n  â˜… Uzbek-specific candidates: {len(uzbek_specific):,} SNPs")
    ...
    print(f"  âœ“ delta_af_all.tsv ({len(daf_out):,} rows)")
    ...
    if len(private_nonafr) > 0:
        ...
        print(f"  âœ“ near_private_variants.tsv ({len(private_nonafr_out):,} rows)")
    ...
    summary_lines = [
        ...
        "",
        "NEAR-PRIVATE VARIANTS (UZB MAFâ‰¥5%):",
        f"  Rare in ALL populations (MAFâ‰¤1%): {len(private_all):,} SNPs",
        f"  Rare in EUR+EAS+SAS (MAFâ‰¤1%): {len(private_nonafr):,} SNPs",
        f"  Enriched in UZB (MAFâ‰¥10%, others MAFâ‰¤3%): {len(enriched_uzb):,} SNPs",
        "",
        "TOP 20 PBS HITS:",
