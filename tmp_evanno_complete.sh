...
MASTER_LOG=$RUNS/evanno_complete.log
SUMMARY_TSV=$RUNS/evanno_completion_summary.tsv
STATUS_TXT=$RUNS/evanno_completion_status.txt

mkdir -p "$RUNS"

echo "[$(date)] Evanno completion started" | tee "$MASTER_LOG"
...
run_one() {
  ...
  local REP="$2"
  local SEED="$3"
  local DIR="$RUNS/K${K}_rep${REP}"
  local REP_LOG="admixture_K${K}_rep${REP}.log"
  ...
  local Q="UZB_for_admixture.${K}.Q"
  local P="UZB_for_admixture.${K}.P"

  if [[ -s "$Q" && -s "$P" && -s "$REP_LOG" ]]; then
    printf '%s\t%s\t%s\tskip_existing\n' "$K" "$REP" "$SEED" >> "$SUMMARY_TSV"
    return 0
  fi

  echo "[$(date)] RUN K=$K rep=$REP seed=$SEED" | tee -a "$MASTER_LOG"
...
echo "[$(date)] Evanno completion finished" | tee -a "$MASTER_LOG"
