#!/usr/bin/env Rscript
# sNMF analysis on Uzbek-only dataset (1,074 samples)
...
BED_DIR  <- "/staging/ALSU-analysis/admixture_analysis"
BED_FILE <- file.path(BED_DIR, "UZB_for_admixture.bed")
OUT_DIR  <- file.path(BED_DIR, "snmf_results_uzbek")
K_RANGE  <- 2:10
N_REPS   <- 10
ALPHA    <- 10
...
vcf_file <- file.path(OUT_DIR, "UZB_for_admixture.vcf")

if (!file.exists(vcf_file)) {
    plink_cmd <- sprintf("plink --bfile %s --recode vcf --out %s/UZB_for_admixture --allow-extra-chr",
                         bed_base, OUT_DIR)
...
# Step 4: Plot
png(file.path(OUT_DIR, "snmf_cross_entropy_uzbek.png"), width = 800, height = 500, res = 120)
plot(ce_summary$K, ce_summary$mean_CE, type = "b", col = "steelblue", pch = 19, cex = 1.2,
     main = "sNMF Cross-Entropy by K (Uzbek-only, n=1074)",
...
for (k in K_RANGE) {
    ...
    out_file <- file.path(OUT_DIR, sprintf("snmf_uzbek_K%d.Q", k))
...
# Step 6: Compare with Uzbek-only ADMIXTURE Q-matrices
cat("\n=== Comparison with ADMIXTURE (Uzbek-only) ===\n")
for (k in c(2, 3, 5)) {
    admix_file <- file.path(BED_DIR, sprintf("UZB_for_admixture.%d.Q", k))
...
# Step 7: JSON
json_lines <- c("{")
json_lines <- c(json_lines, sprintf('  "dataset": "uzbek_only",'))
...
json_lines <- c(json_lines, sprintf('  "n_samples": 1074,'))
...
json_lines <- c(json_lines, "}")
writeLines(json_lines, file.path(OUT_DIR, "snmf_uzbek_results.json"))
cat("JSON saved\n")
