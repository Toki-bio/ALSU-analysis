...
genes.forEach(g => {
  ...
  const name = (g.external_name || g.id).replace('ENSG00000','ENSG..');
  ...
  geneTraces.push({
    ...
    marker:{ size: isPC ? 12 : 8, color: color, symbol: arrowSym },
    hoverinfo:'skip', showlegend:false, xaxis:'x', yaxis:'y2',
  });
...
// Panel 3: rotated LD triangle (LocusZoom-style)
const ldOrder = ldVars.map((v,i)=>({v,i,pos:(snps.find(s=>s.id===v)||{}).pos||0}))
                      .sort((a,b)=>a.pos-b.pos);
const nld = ldOrder.length;
const ldX = [], ldY = [], ldZ = [], ldTxt = [];
for (let i=0;i<nld;i++){
  for (let j=i+1;j<nld;j++){
    const a = ldOrder[i], b = ldOrder[j];
    const r2 = ldMat[a.i][b.i];
    ldX.push((a.pos + b.pos)/2);
    ldY.push(-(j-i));
    ldZ.push(r2);
    ldTxt.push(`${a.v} ↔ ${b.v}<br>r² = ${r2.toFixed(3)}`);
  }
}
const ldTrace = {
  x: ldX, y: ldY, mode: 'markers', type: 'scatter',
  marker: {
    symbol: 'square', size: 9,
    color: ldZ,
  ...
  text: ldTxt, hovertemplate: '%{text}<extra></extra>',
  xaxis: 'x', yaxis: 'y3', showlegend: false, name:'LD',
};
...
const layout = {
  ...
  yaxis:  { domain:[0.58, 1.0], title:'−log₁₀ P', zeroline:false,
            gridcolor:'#eaeef2', ticks:'outside' },
  yaxis2: { domain:[0.42, 0.55], range:[-0.6, nLanes-0.2],
            showticklabels:false, zeroline:false, showgrid:false, ticks:'' },
  yaxis3: { domain:[0.00, 0.40], range:[-(nld-0.5), 0.5],
            showticklabels:false, zeroline:false, showgrid:false, ticks:'' },
  shapes: shapes,
  annotations: [
    ...
    { x: x0 + winSpan*0.01, y: -0.5, xref:'x', yref:'y3', text:'pairwise r² (our cohort)',
      showarrow:false, font:{size:10, color:'#656d76'}, xanchor:'left' },
  ],
  hovermode: 'closest',
};
