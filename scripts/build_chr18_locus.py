...
// These are visual guides connecting each rotated rsID label to its position on the ruler
const connectorLines = snps.map(s => ({
  type:'line', xref:'x', yref:'paper',
  x0:s.pos, x1:s.pos,
  y0:0.40, y1:0.50, // Span from heatmap top to ruler top
  line:{ color:'rgba(100,150,200,0.35)', width:1.5 },
  layer:'below'
}));
...
genes.forEach(g => {
  ...
  // Prefer external_name; fallback to description excerpt or ID
  const displayName = g.external_name || (g.description ? g.description.split('[')[0].trim() : g.id.replace('ENSG00000','ENSG.'));
  ...
  geneTraces.push({
    ...
    hoverinfo: 'text',
    text: `<b>${displayName}</b> (${g.id})<br><i>${btype}</i><br>chr18:${g.start.toLocaleString()}–${g.end.toLocaleString()}<br>strand ${g.strand>0?'+':'−'}`,
  ...
  geneTraces.push({
    ...
    hoverinfo: 'text',
    text: `<b>${displayName}</b> (${g.id})<br><i>${btype}</i><br>strand ${g.strand>0?'+':'−'}`,
    showlegend:false, xaxis:'x', yaxis:'y2',
  });
...
const layout = {
  ...
  yaxis:  { domain:[0.60, 1.0], title:'−log₁₀ P', zeroline:false,
