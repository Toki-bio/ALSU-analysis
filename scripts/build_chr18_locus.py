...
const connectorLines = snps.map(s => ({
  ...
  x0:s.pos, x1:s.pos,
  y0:0.51, y1:0.40, // From ruler to top of heatmap
...
const layout = {
  ...
  yaxis0: { domain:[0.41, 0.51], range:[-0.6, 0.6],
            showticklabels:false, zeroline:false, showgrid:false, ticks:'' },
  yaxis:  { domain:[0.63, 1.0], title:'−log₁₀ P', zeroline:false,
            gridcolor:'#eaeef2', ticks:'outside' },
  yaxis2: { domain:[0.52, 0.62], range:[-0.6, nLanes-0.2],
            showticklabels:false, zeroline:false, showgrid:false, ticks:'' },
